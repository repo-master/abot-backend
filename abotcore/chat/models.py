...
from sqlalchemy import (JSON, Column, DateTime,
                        Identity, Integer, String, Enum, select)
...
class ChatHistory(AbotBase):
    ...
    chat_handler: Mapped[str] = Column(
        'chat_handler',
        String
    )
    message_client_id: Mapped[str] = Column(
        'message_client_id',
        String,
        nullable=True
    )
    message_role: Mapped[ChatRole] = Column(
        'message_role',
        Enum(ChatRole),
        default=ChatRole.HUMAN
    )
    message_content: Mapped[str] = Column(
        'message_content',
        String,
        nullable=True
    )
...
class UserChatMemory(AbotBase):
    ...
    memory_data: Mapped[List[Memory]] = Column(
        'memory_data',
        JSON,
        default=[]
    )
